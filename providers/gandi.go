...
func (g *GandiHandler) newZoneVersion() (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("Failed to refresh zone information: %v", err)
	}
	...
	// Store base version
	g.baseVersion = zoneInfo.Version
...
func (g *GandiHandler) setZoneVersion(version int64) (error) {
	...
	if zoneInfo.Version != g.baseVersion {
		...
		if err != nil {
			return fmt.Errorf("Failed to delete conflicting zone version %v. Remove it manually.", version)
		}
